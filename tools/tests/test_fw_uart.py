"""L3-UART-TX and L3-UART-RX on the golden model, through the host port.

``firmware/uart_tx_fifo.loom`` and ``firmware/uart_rx.loom`` are assembled,
loaded, configured and fed entirely through ``tools.loomhost.Loom`` over
``ModelTransport`` (SPI bytes, 64 clocks each, firmware running meanwhile),
with the UART reference models of ``tools.protomodels.uart`` on the pads.
...
TICK 8) and at 115200 baud (TICK 54 + 64/256).
"""
...
def boot(bench, name, tick_int, tick_frac=0):
...
# ----------------------------------------------------------------------- TX
def test_uart_tx_sends_a_host_pushed_string():
    bench = Bench()
    rx = bench.add(UartRx("OUT0", 32))
    loom, _ = boot(bench, "uart_tx_fifo", 32)
...
def test_uart_tx_at_115200_baud_edges_within_one_slot():
    bits = 434 + 8 / 256                                   # 50 MHz / 115200 = 434.03
    bench = Bench()
    rx = bench.add(UartRx("OUT0", bits))
    loom, _ = boot(bench, "uart_tx_fifo", 434, 8)
    loom.push(0, b"U\x00")
...
def test_uart_tx_ignores_the_high_byte_of_a_word():
    bench = Bench()
    rx = bench.add(UartRx("OUT0", 32))
    loom, _ = boot(bench, "uart_tx_fifo", 32)
...
# ----------------------------------------------------------------------- RX
def rx_bench(tick_int, tick_frac, bit_clocks, gap_bits=0.0):
    bench = Bench()
    tx = bench.add(UartTx("IN0", bit_clocks, gap_bits=gap_bits))
    loom, program = boot(bench, "uart_rx", tick_int, tick_frac)
...
def test_uart_rx_delivers_bytes_and_a_framing_error_in_bit_8():
    bench, tx, loom, _ = rx_bench(8, 0, 64)
...
def test_uart_rx_samples_every_bit_near_its_middle():
    bench, tx, loom, program = rx_bench(8, 0, 64)
...
def test_uart_rx_tolerates_three_per_cent_at_the_fastest_rate(skew):
    """64 clocks per bit, one idle bit between frames (see the program header)."""
    bench, tx, loom, _ = rx_bench(8, 0, 64 * skew, gap_bits=1)
    for chunk in (b"\x00\xff\x5a\xa5", b"\x81\x7e\x33\xcc"):
        send_and_wait(bench, tx, chunk)
        assert loom.pop(0, 4) == list(chunk)


def test_uart_rx_at_115200_baud():
    bench, tx, loom, _ = rx_bench(54, 64, 434 + 8 / 256)   # 54.25 clocks per tick
    send_and_wait(bench, tx, b"OK!")
    assert loom.pop(0, 3) == [ord(c) for c in "OK!"]


@pytest.mark.parametrize("skew", [0.97, 1.03])
def test_uart_rx_back_to_back_at_115200_with_three_per_cent_error(skew):
    bench, tx, loom, _ = rx_bench(54, 64, (434 + 8 / 256) * skew)
    send_and_wait(bench, tx, b"\x00\xff\x5a\xa5")
    assert loom.pop(0, 4) == [0x00, 0xFF, 0x5A, 0xA5]


def test_uart_rx_flags_an_overrun_in_bit_9():
    bench, tx, loom, _ = rx_bench(8, 0, 64)
...
def test_uart_rx_ignores_a_glitch_and_a_break_is_one_error():
    bench, tx, loom, _ = rx_bench(8, 0, 64)
